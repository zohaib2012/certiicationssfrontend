...
<div dir="rtl" className="space-y-10 text-right">
  ...
  <Section title='بيانات الشهادة:' >
    <KeyValue label="رقم الشهادة" value={certificate.certificateNumber.substring(0,20)  .substring(0,20)  } />
    <KeyValue label="تاريخ الإصدار" value={certificate.issueDate.substring(0,20)  .substring(0,20)  } />
    <KeyValue label="نهاية الصلاحية" value={certificate.expiryDate.substring(0,20)  .substring(0,20)  } />
  </Section>

  <Section title="بيانات صاحب الشهادة: ">
    <KeyValue label="الاسم" value={certificate.customerName.substring(0,20)  .substring(0,20)  } />
    <KeyValue label="رقم الهوية" value={certificate.idNumber.substring(0,20)  .substring(0,20)  } />
    <KeyValue label="الجنس" value={certificate.gender.substring(0,20)  .substring(0,20)  } />
    <KeyValue label="الجنسية" value={certificate.nationality.substring(0,20)  .substring(0,20)  } />
    <KeyValue label="المهنة" value={certificate.profession.substring(0,20)  .substring(0,20)  } />
    <KeyValue label="البرنامج التثقيفي" value={certificate.educationalProgram.substring(0,20)  .substring(0,20)  } />
    <KeyValue label="تاريخ إنهاء البرنامج" value={certificate.programCompletionDate.substring(0,20)  .substring(0,20)  } />
  </Section>
...
const KeyValue = ({ label, value }) => (
  ...
  <div className="flex justify-evenly ml-40 space-x-20 py-2  last:border-none border-2 border-amber-500">
